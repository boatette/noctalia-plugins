...
On a side bar the grid is transposed rather than rotated — digits run down the bar, each digit's four bits run across it with 8 on the left — so the weights keep a fixed reading order whichever edge the bar is docked to.
